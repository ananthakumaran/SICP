...
;; has a dot before the last parameter name indicates that, when the
;; procedure is called, the initial parameters 
;; (if any) will have as values the initial arguments, as usual, but the
;; final parameter's value will be a list of any remaining arguments. For
...
(define (filter lat predicate?)
  (define (iter lat result)
    (if (null? lat)
	result
	(iter (cdr lat) 
	      (if (predicate? (car lat))
		  (cons (car lat) result)
		  result))))
  (iter lat '()))
