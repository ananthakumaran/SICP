;; Exercise 2.42. Figure 2.8: A solution to the eight-queens puzzle.  The
...
;; in figure 2.8. One way to solve the puzzle is to work across the
...
;; approach recursively: Assume that we have already generated the
...
;; of positions by placing a queen in each row of the kth column. Now
;; filter these, keeping only the positions for which the queen in the
;; kth column is safe with respect to the other queens. This produces the
...
;; all solutions to the puzzle.  We implement this solution as a
;; procedure queens, which returns a sequence of all solutions to the
;; problem of placing n queens on an n× n chessboard. Queens has an
;; internal procedure queen-cols that returns the sequence of all ways to
...
(define (for-all predicate seq)
  (define (recur seq)
    (if (null? seq)
	true
	(and (predicate (car seq))
	     (recur (cdr seq)))))
  (recur seq))
...
(define (safe? positions)
  (let ((new (car positions)))
    (for-all (lambda (other)
	       (and (same-row? new other)
		    (same-column? new other)
		    (diagonal? new other)))
	     (cdr positions))))

(define (same-row? new other)
  (not (= (row other) (row new))))
(define (same-column? new other)
  (not (= (col other) (col new))))
(define (diagonal? new other)
  (not (= (abs (- (row other) (row new))) (abs (- (col other) (col new))))))
